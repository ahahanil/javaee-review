...
## 1.1. 简介
`Feign`也叫伪装。`Feign`可以把Rest的请求进行隐藏，伪装成类似SpringMVC的Controller一样。不用再自己拼接url，拼接参数等等
操作，一切都交给`Feign`去做。[项目主页](https://github.com/OpenFeign/feign)
## 1.2. 快速入门
### 1.2.1. 导入依赖
...
### 1.6. 日志级别(了解)
前面讲过，通过 `logging.level.xx=debug` 来设置日志级别。然而这个对Feign客户端而言不会产生效果。因为 `@FeignClient` 注解修改的客户端在被代理时，都会创建一个新的`Fegin.Logger`实例。需要额外指定这个日志的级别才可以。
...
`Spring Cloud Gateway`组件的核心是一系列的过滤器，通过这些过滤器可以将客户端发送的请求转发（路由）到对应的微服务。 `Spring Cloud Gateway`是加在整个微服务最前沿的防火墙和代理器，隐藏微服务结点IP端口信息，从而加强安全保护。`Spring Cloud Gateway`本身也是一个微服务，需要注册到Eureka服务注册中心。
...
## 2.3. 核心概念
`route`路由信息的组成：由一个ID、一个目的URL、一组断言工厂、一组Filter组成。如果路由断言为真，说明请求URL和配置路由匹配。

`Predicate`断言 `Spring Cloud Gateway`中的断言函数输入类型是`Spring 5.0`框架中的
`ServerWebExchange`。`Spring Cloud Gateway`的断言函数允许开发者去定义匹配来自于`Http Request`中的任何信息比如请求头和参数。
...
本例中，将路径中包含有 `/user/**` 开头的请求，代理到`http://127.0.0.1:9091`
...
路由配置中`uri`所用的协议为`lb`时（以`uri: lb://user-service`为例），`gateway`将使用 `LoadBalancerClient`把`user-service`通过eureka解析为实际的主机和端口，并进行ribbon负载均衡。
...
## 2.6. 路由前缀
### 2.6.1. 添加前缀
在`gateway`中可以通过配置路由的过滤器`PrefixPath`，实现映射路径中地址的添加修改 application.yml 文件
```yaml
server:
  port: 10010
spring:
...
测试访问：`http://localhost:10010/api/user/8?name=itcast` 检查后台是否输出`name`和`itcast`；但是若访问 `http://localhost:10010/api/user/8?name2=itcast` 则是不会输出的。
...
码云还提供了项目管理、代码托管、文档管理的服务。本章中使用的远程Git仓库是码云。

码云[访问地址](https://gitee.com/)
...
修改在远程仓库上的`user-dev.yml`文件，添加一个属性`test.name`。
...
Spring Cloud Bus是**用轻量的消息代理将分布式的节点连接起来**，可以用于广播配置文件的更改或者服务的监控管理。也就是**消息总线可以为微服务做监控，也可以实现应用程序之间相互通信**。 Spring Cloud Bus可选的消息代理有`RabbitMQ`和`Kafka`。
- 使用了Bus之后
