...
- 同步消息过程。消息同步还是异步通过`ClientCnxn#submitRequest()`内部调用`ClientCnxn#queuePacket()`方法第五个参数`AsyncCallback`决定同步时`AsyncCallback`为`null`异步时这里是`AsyncCallback`回调对象
  - 同步时直接实例化了`Packet`对象放入`outgoingqueue`然后`ClientCnxnSocketNIO`的`doIO`就可写了,首先从`outgoingqueue`获取刚刚存放的`Packet`然后发送出去，由于不存在requestHeader所以也会把`Packet`放入`pendingqueue`中
  
  ![packet消息包](assets/packet消息包.png)
  - 然后就是等待远程服务端相应业务消息，然后从`pendingqueue`取出`Packet`消息的`zxid`和服务端响应对比不一致即消息紊乱了抛出异常，否者就会反序列化响应消息最后重置`Packet`对象`finished`状态
  - `ClientCnxn#submitRequest()`内部调用`ClientCnxn#queuePacket()`方法后执行上面几步逻辑后`ClientCnxn#submitRequest()`就处于轮询判断`Packet`对象`finished`状态是否true完成，完成即返回。在`ClientCnxn#finishPacket()`方法中通过重置`Packet`对象`finished`状态来标记同步或异步操作完成，重置是如果是异步会唤醒等待线程而同步时会通过`ClientCnxn#queuePacket()`方法把完成消息添加到`waitingEvents`事件队列里
  - 在`EventThread#run()`中一直在轮询判断异步相关操作

  ![消息传递](assets/消息传递.png)

- 事件通知消息流转
  - `Zookeeper#getData()`中会通过事件和节点实例化`DataWatchRegistration`对象
  ...
  - 然后在调用`ClientCnxn#finishPacket()`方法重置`Packet`对象`finished`状态之前会判断是否注册了事件，当注册了`DataWatchRegistration`事件就会执行`Zookeeper#register()`方法取出`DataWatchRegistration`把事件和节点k/v映射放入Map中就完成了注册事件。
  - 然后当事件发生了就会发送一个`xid=-1`通知会在`ClientCnxn#readResponse()`收到这个notification通知然后进行反序列化，然后通过`ClientCnxn#queuePacket()`方法把事件进行一些封装逻辑(比如通过`Zookeeper#materialize()`当事件发生后把之前注册的事件删除即监视器是一次性的)后添加到`waitingEvents`事件队列里通过`EventThread#run()`轮询时满足条件后调用`ClientCnxn#prosessEvent()`处理事件的回调即会回调注册监视器时回调方法
